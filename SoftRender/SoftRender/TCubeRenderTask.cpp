...
TCubeRenderTask::TCubeRenderTask(TBasicWindow& win)
    : m_angle(0)
{
	float vertices[] = {
		// Front face
...
void TCubeRenderTask::Render(TSoftRenderer& sr)
{
    Transform();
    sr.ClearColor({ 0,0,0 });
    sr.ClearDepth(1.0f);
...
void TCubeRenderTask::Transform()
{
    m_angle -= 0.1f;
    m_shader.modelMatrix = tmath::RotationMatrix(tmath::Vec3f(1.0f, 1.0f, 1.0f), m_angle);
}